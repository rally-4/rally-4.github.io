...
local w = 760
local h = 976
local rx = fmath.random_int(0,512)/32
local ry = fmath.random_int(0,512)/32
local Vertices = {}
local Colors = {}
local Segments = {}
local i = 0
...
for x=0,w,8 do
    for y=0,h,48 do
        z=16*perlin:noise(x/32+rx,y/32+ry)
        table.insert(Vertices, {x,y-z,z})
        table.insert(Vertices, {x+4,y-z,z})
        table.insert(Vertices, {x+4,y+4-z,z})
        table.insert(Vertices, {x,y+4-z,z})
        table.insert(Segments, {i,i+1,i+2,i+3,i})
        i=i+4
        color=0x00ff0000+math.floor(255*math.abs(perlin:noise(x/32+rx,y/32+ry)))
