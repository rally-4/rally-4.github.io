window.addEventListener('DOMContentLoaded', ()=>{
    ...
    let CRAWL = new EnemyAttack('CRAWL', 20, 3);
    let CUT = new EnemyAttack('CUT', 80, 1, 75);
    const ScrapSlimeAttacks = [CRAWL, CUT, ENGULF];
    let ADVANCE = new EnemyAttack('ADVANCE', 25, 3, 50);
    let DELUGE = new EnemyAttack('DELUGE', 50, 3, 0, 0, 100, 100);
    let RECOVER = new EnemyAttack('RECOVER', 100, 5, 0, 0, 0, 500, 250);
        ...
        let BEGIN = function(){
            ...
            setInterval(()=>{
                if(c){
                    ...
                    if(COH <= 0){
                        obj.res.materials = mi + Number(obj.res.materials);
    ...
    E1.addEventListener('click', ()=>{
        if(A1Wins < A1BossWins){
            if(Math.random() < .5){
                ...
                LoadFight('Scrap Slime', 350, 570, 10, .1, UserHealth, UserShield, ShieldDowntime, ShieldRegen, ScrapSlimeAttacks, 2, 'A1');
            }
        }else{
            LoadFight('Reinforced Slime', 1000, 750, 15, .2, UserHealth, UserShield, ShieldDowntime, ShieldRegen, ReinforcedSlimeAttacks, 5, 'A1B');
        }
    });
});
